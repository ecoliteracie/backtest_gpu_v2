...
def main() -> int:
    ...
    # ---- Phase 11: GPU Event Pairing + CPU Pricing — FULL GRID SUMMARY ----
    from src.sim_core import simulate_grid_summary
    ...
    try:
        ...
        for label in labels:
            out_csv = simulate_grid_summary(
                df=df,
                rsi_maps=RSI_MAPS,
                regime_label=label,
                cfg=cfg,
                out_dir="results",
                backend="auto",         # numba if available; falls back to CPU for events if needed
                batch_size=None,        # let the function pick a safe default or override via cfg["GRID_BATCH_SIZE"]
            )

            # Optional banner per label
            meta11 = {
                "grid": {"regime": label},
                "gpu": gpu_info,
                "out": {"summary_csv": out_csv},
            }
            banner = (
                f"\n====================================================\n"
                f"Phase 11 — Grid Summary\n"
                f"====================================================\n"
                f"[REGIME] {label}\n"
                f"[OUT   ] {out_csv}\n"
            )
